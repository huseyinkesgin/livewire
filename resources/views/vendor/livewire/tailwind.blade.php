        ...
        <div class="flex justify-between flex-1 sm:hidden">
            <span>
                @if ($paginator->onFirstPage())
                    <span class="relative inline-flex items-center px-2 py-1 text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-4 rounded-md select-none">
            ...
            <span>
                ...
                @else
                    <span class="relative inline-flex items-center px-2 py-1 ml-3 text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-4 rounded-md select-none">
                        {!! __('pagination.next') !!}
                    </span>
                @endif
            </span>
                    ...
                    <span>
                        ...
                        @if ($paginator->onFirstPage())
                            <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                <span class="relative inline-flex items-center px-1 py-1 text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-l-md leading-4" aria-hidden="true">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span class="relative inline-flex items-center px-2 py-1 -ml-px text-xs font-medium text-gray-700 bg-white border border-gray-300 cursor-default leading-4 select-none">{{ $element }}</span>
                            </span>
                        @endif
                                ...
                                <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                    @if ($page == $paginator->currentPage())
                                        <span aria-current="page">
                                            <span class="relative inline-flex items-center px-2 py-1 -ml-px text-xs font-medium text-white bg-indigo-600 border border-indigo-600 cursor-default leading-4 select-none hover:bg-indigo-600">{{ $page }}</span>
                                        </span>
                                    @else
                    ...
                    <span>
                        ...
                        @else
                            <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                <span class="relative inline-flex items-center px-1 py-1 -ml-px text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-r-md leading-4" aria-hidden="true">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                                    </svg>
                                </span>
