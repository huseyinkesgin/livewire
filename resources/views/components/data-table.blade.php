...
<div x-data="{
    confirmDelete(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Livewire.dispatch('deleteRow', id);
            }
        });
    }
    ...
    <div class="flex items-center space-x-2 px-4 py-2 bg-white border-b">
        <button type="button" class="action-md" title="Yeni" wire:click="$dispatch('openCreateModal')">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
            </svg>
        </button>
        <button type="button" class="action-md" title="Düzenle" @if($activeRow) wire:click="$dispatch('editRow', { id: {{ $activeRow }} })" @endif>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
        </button>
        <button type="button" class="action-md" title="Sil" @if($activeRow) x-on:click="confirmDelete({{ $activeRow }})" @endif>
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button type="button" class="action-md" title="Yenile">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mt-auto px-4 py-2 bg-gray-50 border-t border-gray-200">
        <div class="flex items-center justify-between">
            <div class="text-xs text-gray-700">
                {{ $records->firstItem() ?? 0 }} ile {{ $records->lastItem() ?? 0 }} arası gösteriliyor, toplam {{ $records->total() }} kayıt
            </div>
            <div>
                {{ $records->links() }}
            </div>
